...
function createEventMarkers()
{
    ...
    {
        switch(events[i].location)
        {
            case 'University Memorial Center': location = 'umc'
            break
            case 'Engineering Center': location = 'eccr'
            break
            case 'Mathematics': location = 'math'
            break
            case 'Rec Center': location = 'rec'
            break
            case 'Farrand Field': location = 'farrand'
            break
            case 'Norlin': location = 'norlin'
            break
            case 'Eaton Hummanities': location = 'humanities'
            break
            case 'Hellems': location = 'hellems'
            break
        }
        for(k = 0; k < positions.length; k++)
        {
            if(location == positions[k].name)
            {
                var coordinates = new lat_lng(positions[k].lat, positions[k].lng)
...
function createEventInfoWindow(marker,event)
{
    ...
    {
        document.getElementById('eventTitle').innerHTML = event.event   
